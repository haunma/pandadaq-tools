/* pdaqtool.c - PandaDAQ board loader                                */
/* Based on bfpgatool.c, copyright 2010 Eric Brombaugh               */
...
{
    fprintf(stderr,
        "Usage: pdaqtool [-f freq][-r addr][-w addr data][-v][-V] [BITSTREAM] \n"
...
{
    pdaq *bs;
    int flags = 0, verbose = 0, read = 0, write = 0;
    int addr = 0, data = 0;
    ...
    while (1+flags < argc && argv[1+flags][0] == '-')
    {
        ...
        case 'V':
            fprintf(stderr, "pdaqtool version %s\n", VERSION);
            exit(0);
        default:
    ...
    /* open up hardware */
    if((bs = pdaq_init(4, 0x50, 0x20, 1, 0, verbose)) == NULL)
    {
        fprintf(stderr, "Couldn't access hardware\n");
        exit(2);
    }
    
    /* send bitstream if bitfile name available */
    if(argc > flags + 1)
    {
        if(pdaq_cfg(bs, argv[flags + 1]))
        {
            fprintf(stderr, "Error sending bitstream\n");
        }
    }
    ...
    if(read || write)
    {
        ...
        /* send a message */
        ret = pdaq_spi_txrx(bs, tx, rx, ARRAY_SIZE(tx));
        if (ret == 1)
        {
